...
fn create_release<G>(
    ...
    profile: &Profile,
    signer: &Device<G>,
) -> Result<ReleaseId, error::CreateRelease>
where
    ...
    let reuse: Option<ReleaseId> = match resolved.tag {
        ...
        None => match mine.as_slice() {
            ...
            _ => {
                ...
                match prompt::pick_release_or_create(&mine, None, repo, profile)
                    .map_err(error::CreateRelease::Usage)?
                {
                    prompt::ReleaseChoice::Existing(id) => Some(id),
                    // With no tag the picker offers no "create new" entry,
                    // so this arm is effectively unreachable; mint to be total.
                    prompt::ReleaseChoice::CreateNew => None,
                }
            }
        },
    };
...
mod error {
    ...
    pub enum CreateRelease {
        ...
        #[error(transparent)]
        Find(#[from] Find),
        #[error("commit {oid} has {} release(s) you authored; re-run naming the tag to pick one (e.g. `create <tag>`) (candidates: {})", candidates.len(), display_ids(candidates))]
        Ambiguous {
            oid: Oid,
