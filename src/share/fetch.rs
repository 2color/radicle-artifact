...
//!
//! Both transports enforce a connect timeout and an idle (no-progress)
//! timeout so unreachable or stalled providers fail fast instead of hanging
//! the caller. Per-provider iroh causes are not preserved (the
//! [`iroh_blobs`] downloader intentionally drops them on
//! `ProviderFailed`); set `RUST_LOG=iroh_blobs=debug` for detail.
...
/// window is abandoned so the next one is tried.
...
/// Build a ureq agent with our connect/read timeouts applied.
...
pub fn download_collection(
    ...
    let (iroh_ids, _urls) = partition_locations(locations);
    if iroh_ids.is_empty() {
        // No iroh providers and HTTP collection fetch is unsupported — no
        // attempt can be made, so this is distinct from "all attempts failed".
        return Err(Error::NoLocations);
    }
