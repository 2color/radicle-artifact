...
fn seeded_tag(rid: &RepoId, release: &ReleaseId, cid: &Cid) -> Vec<u8> {
...
fn seeded_release_prefix(rid: &RepoId, release: &ReleaseId) -> Vec<u8> {
    let mut out = seeded_rid_prefix(rid);
    let rel = release.oid();
    let rel_b = AsRef::<[u8]>::as_ref(&rel);
...
/// Inverse of [`seeded_tag`]: decode a tag name into `(RepoId, ReleaseId, Cid)`.
fn parse_seeded_tag(name: &[u8]) -> Option<(RepoId, ReleaseId, Cid)> {
    ...
    let release = ReleaseId::from(oid_from_bytes(rel_b)?);
...
pub async fn all_seeded(store: &Store) -> Result<Vec<(RepoId, ReleaseId, Cid, Hash)>, Error> {
    let mut stream = store
        .tags()
...
mod tests {
    ...
    /// A `ReleaseId` from a 40-hex-char (20-byte SHA-1) Oid.
    ...
    fn all_seeded_round_trip() {
        ...
        rt.block_on(async {
            ...
            let got: HashSet<(RepoId, ReleaseId, Cid, Hash)> =
                all_seeded(&store).await.unwrap().into_iter().collect();
            let want: HashSet<(RepoId, ReleaseId, Cid, Hash)> = triples
                .into_iter()
    ...
    fn seeded_tag_layout() {
        ...
        let rel_oid = rel.oid();
        assert_eq!(&tag[rel_off..cid_off], AsRef::<[u8]>::as_ref(&rel_oid));
