...
Registering an artifact from a local `<PATH>` now also records a `size-bytes` metadata entry, so peers can see how large an artifact is before fetching. The size matches the seeder's logical size (a file's length, or the sum of a directory's member files) and is written in the same signed COB entry as the CID. Pass `--no-size` to skip it; registering by `--cid` records no size since there are no local bytes to measure.
